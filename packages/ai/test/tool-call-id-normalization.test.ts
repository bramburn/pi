 ...
 *
 * 1. Use github-copilot gpt-5-mini to generate a tool call
 * 2. Switch to openrouter openai/gpt-5.1 and complete
...
describe("Tool Call ID Normalization - Live Handoff", () => {
	it.skipIf(!copilotToken || !openrouterKey)(
		...
		async () => {
			const copilotModel = getModel("github-copilot", "gpt-5-mini");
			const openrouterModel = getModel("openrouter", "openai/gpt-5.1");
...
describe("Tool Call ID Normalization - Prefilled Context", () => {
	...
	it.skipIf(!openrouterKey)(
		...
		async () => {
			const model = getModel("openrouter", "openai/gpt-5.1");
