...
function pruneContent(
	...
	const _removed = totalChars - config.headChars - config.tailChars;
	const out: unknown[] = [];
	for (const block of blocks) {
...
export function pruneSession(
	...
	for (const m of messages) {
		...
		// AND the per-message `excludeFromPrune` flag (decision matrix
		// item 10: the `read` tool sets `excludeFromPrune: true` on its
		// result so the pruner is a no-op for it; an extension can also
		// opt out per message).
