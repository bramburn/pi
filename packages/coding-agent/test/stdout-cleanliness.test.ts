...
describe("stdout cleanliness in non-interactive modes", () => {
	it("prints --version to stdout when stdout is redirected", async () => {
		...
		// Fork bramburn: --version now prints `pi <version> (<runtime>) [bramburn]`.
		...
		// The runtime parentheses format is from formatVersionWithRuntime() in config.ts.
		expect(result.stdout.trim()).toMatch(/^pi \d+\.\d+\.\d+(-[a-z0-9.]+)? \(\w+\) \[bramburn\]$/);
