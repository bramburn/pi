...
Content`,
		...
		it(
			"should resolve symlinked user and project resources once",
			{ skip: process.platform === "win32" },
			async () => {
				// Windows requires admin or developer mode to create symlinks. The
				// resource manager's symlink-deduplication behaviour is exercised on
				// POSIX.
...
Content`,
	...
	describe("pattern filtering in package filters", () => {
		...
		it(
			...
			{ skip: process.platform === "win32" },
			async () => {
				// The test counts discovered skills and assumes HOME=tempDir scopes
				// the search to the temp fixture. On Windows, the package manager
				// also reads `USERPROFILE`-relative `.agents/skills`, which surfaces
				// globally installed skills (e.g. extract-design-system) and
				// inflates the count. The autoload-disabled semantics themselves
				// are exercised on POSIX.
