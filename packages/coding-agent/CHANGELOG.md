...
- Synced upstream v0.84.2 (fullscreen transcript search, default tools, fullscreen exit output, AGENTS.override.md, Mistral HTTP transport, harness v2, session v4, ModelsRequestTransforms, context.publish, undici 8.9.0, brace-expansion 5.0.9, nanoid security update).
- Version bumped to `0.84.2-b1` to mark the first fork build on the v0.84.2 baseline.
- Kept `FORK_NAME="bramburn"` marker in `pi --version` output (re-applied after upstream rewrite of `config.ts` and `main.ts`).
- Kept per-sibling `toolResult` emission in parallel tool batches (no orphans when one sibling stalls), re-applied on top of upstream's `executeToolCallsParallel` rewrite in `packages/agent/src/agent-loop.ts`.
- Kept the fork-specific `validateLlmMessages()` extension-transform check in `packages/agent/src/agent-loop.ts`; the new regression test `validate-llm-messages.test.ts` is restored alongside the call site.
- Reverted upstream's `toolResultIds` expectation in `agent-loop.test.ts` to `["tool-2", "tool-1"]` to match the fork's per-sibling emission order.
- Reverted upstream's `stdout-cleanliness.test.ts` regex to `/^pi \d+\.\d+\.\d+ \[bramburn\]$/` so CI catches accidental removal of the fork marker.
- The fork's pre-v0.84 scrollback-jump fix targeted the old class-based `TuiMainScreen`; upstream v0.84.2 rewrote the TUI in a functional architecture that gates `fullRender(true)` on `firstChanged < prevViewportTop`, which already prevents the same class of scrollback jumps. The fork fix is therefore obsolete and intentionally not re-applied.
