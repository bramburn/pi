...
function getInternalWorkspaces(lockPackages) {
	...
	for (const [lockPath, entry] of Object.entries(lockPackages)) {
		...
		// package.json files still use `@earendil-works/pi-*`. The install
		// lock must therefore resolve either spelling back to the same
		// workspace directory.
...
function generateInstallLock() {
	...
	while (queue.length > 0) {
		...
		if (workspace) {
			...
			// package's deps) so the install-lock output's
			// `node_modules/<name>` paths and resolved tarball URLs match
			...
			if (!addedPaths.has(outputPath)) {
				addInternalWorkspace(installLockPackages, addedPaths, queue, outputName, workspace);
			}
			continue;
		}
